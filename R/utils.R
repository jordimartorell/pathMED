...
.methodsML <- function(algorithms,
                   ...
                   outcomeClass,
                   training){
  availableMethods <- list(numeric=c('glm','lm','xgbTree','rf','knn','nnet',
                                     'svmRadial','svmLinear','lars','rpart',
                                     'gamboost','brnn','enet'),
                         character=c('glm','lda','xgbTree','rf','knn','nnet',
                                       'svmLinear','svmRadial','nb','ada',
                                       'gamboost'))
  methodList <- list(
    ...
    xgbTree=caretModelSpec(method='xgbTree',
                           tuneGrid=expand.grid(.max_depth = c(2, 3, 4, 5, 6, 8,
                                                               10),
                                                .nrounds = 50,
                                                .eta = c(0.01, 0.05, 0.1),
                                                .gamma = c(0,1),
    ...
    nb=caretModelSpec(method='nb',
                      tuneGrid=expand.grid(.fL=c(0, 0.5, 1.0), .usekernel=TRUE,
                                           .adjust=c(0.5, 1.0))),
    ...
    enet=caretModelSpec(method='enet',
                        tuneGrid=expand.grid(.lambda=c(0, 0.005, 0.009, 0.05,
                                                       0.1),
