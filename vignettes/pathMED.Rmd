...
**algorithms** parameter (see the function help). Subsampling is performed n
times (**subsamples**) to split data into training and test datasets, with the
proportion of training samples specified in the **splitProp** parameter. With
training data, n-fold cross-validation (n=**foldsCV**) is repeated several times
(**reaptsCV** parameter) for hyperparameter tuning. An optional features filter
may be applied (**featureFilter**) parameter. **Prior** parameter is used to
specify the metric to select the best model.
