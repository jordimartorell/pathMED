...
We loaded `refData` into the environment, which includes four gene
expression datasets and four corresponding metadata datasets containing
clinical information. These datasets were extracted from NCBI GEO and
contain both lupus and healthy samples, encoded in the metadata as
"Healthy_sample" and "Disease_sample".
...
a matrix with the cases samples, gene sets or the output of the
`mScores_filterPaths` function and the output of
`mScores_createReference()`. The M-scores are calculated using the *k*
most similar samples of the reference data. *k* may be specified with
the `nk` parameter. The `distance.threshold` parameter stablish the
euclidean distance that a sample may have with the reference as maximum
to impute its M-score. The output is a matrix with the M-score of each
gene set (rows) and sample (columns).
