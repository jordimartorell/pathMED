...
## 2.1. Build the reference data object
...
## 2.2. Calculate the reference M-scores
...
refMscore <- mScores_createReference(refData,
                           geneSets="tmod",
                           cores=1)
```

## 2.3. Filter gene sets
...
```{r, warning=FALSE}
relevantPaths <- mScores_filterPaths(MRef=refMscore,
                       min_datasets=3,
                       perc_samples=10)
```

## 2.4. Calculate the M-scores
