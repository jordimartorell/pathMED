...
geneset and sample. The input is a matrix or data frame with molecular features 
in rows and samples in columns, as well as a geneset collection. The geneset 
may be selected from the preloaded ones (Gene Ontology, KEGG, Reactome and 
tmod), or a custom geneset may be supplied as a named list.
The function has implemented 22 scoring methods, which may be
selected with the **method** parameter. The available methods and
references are:
...
calculates the M-scores for each gene set of the specified database. By
default, Reactome is the used database, but the **genesets** parameter
can be modified to use KEGG ("kegg"), transcriptional modules related
with immune functions ("tmod") or Gene Ontology ("gobp", "gomf",
"gocc"). Furthermore, if "custom" is selected, any geneset with the
proper format can be introduced to the **customGeneset** parameter. It
should be a named list with the genes of each gene set. If you are
running the script with a multi-core processor, use the **cores**
parameter to specify the number of workers to be used (most of the
pathMED functions include this parameter).
...
**getML()** function fits and tests several machine learning algorithms
in order to find which one has the best performance predicting the
specified variable (**var2predict** parameter). The input objects are
the M-scores matrix calculated in the step 2 (**expData**), and a data
frame with the metadata of the samples (**metadata**). One or several
algorithms may be selected with the **algorithms** parameter (see the
function help). Nested cross-validation is performed using k folds for
the outter CV (**Koutter**) and inner CV (**Kinner**). The inner k-fold
